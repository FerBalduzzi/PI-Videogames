...
const getApiInfo = async () => {
  let videogamesArr = [];
  let urlApi = `https://api.rawg.io/api/games?key=${YOUR_API_KEY}`;
  try {
    for (let i = 0; i < 5; i++) {
      const urlData = await axios.get(urlApi); //paso los juegos y los juegos son muchos objetos
      urlData.data.results //Guardo la respuesta de results (results es un array con esos objetos)que son los objetos de videojuegos
        .map((e) => {
          //le tiro un map para que me pase un nuevo array solo con esas propiedades
          videogamesArr.push({
            id: e.id,
            name: e.name,
    ...
    return videogamesArr; //retorno ese array nuevo
  } catch (error) {
...
const dbId = async (id) => {
  try {
    const dbGame = await Videogame.findByPk(id, { include: Genre }); //que busque el que coincida con ese id que me pasan con pk lo hago
    return {
      id: dbGame.id,
